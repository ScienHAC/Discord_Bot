...
const pool = new Pool({
    connectionString: process.env.DATABASE_URL, // Ensure you set DATABASE_URL in your .env file
});
...
client.on('guildCreate', async (guild) => {
    console.log(`Joined a new guild: ${guild.name} (ID: ${guild.id})`);
    await pool.query(`INSERT INTO channels (guildId) VALUES ($1) ON CONFLICT (guildId) DO NOTHING`, [guild.id]);
    registerCommandsForGuild(guild.id); // Register commands for the new guild
});
...
const upsertChannel = async (guildId, channelId, checkInterval, deleteTime) => {
    await pool.query(
        `INSERT INTO channels (guildId, channelId, checkInterval, deleteTime) VALUES ($1, $2, $3, $4)
        ON CONFLICT (guildId, channelId) DO UPDATE SET checkInterval = excluded.checkInterval, deleteTime = excluded.deleteTime`,
        [guildId, channelId, checkInterval, deleteTime]
    );
};
...
const removeChannel = async (guildId, channelId) => {
    await pool.query(`DELETE FROM channels WHERE guildId = $1 AND channelId = $2`, [guildId, channelId]);
};

// Helper function to fetch all channels and their settings
const getChannelsForGuild = async (guildId) => {
    const res = await pool.query(`SELECT channelId, checkInterval, deleteTime FROM channels WHERE guildId = $1`, [guildId]);
    return res.rows;
};
...
const checkOldMessages = async () => {
    const now = Date.now();
    const res = await pool.query(`SELECT DISTINCT guildId, channelId, checkInterval, deleteTime FROM channels`);
