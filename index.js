...
bot.on("ready", async () => {
  ...
  await setupIntervals(); // Setup intervals for message deletion
});
...
// Function to handle /check-gravbits command
async function handleCheckGravbits(guildId, channelId, newInterval) {
  try {
    await pgClient.query(`
      UPDATE gravbits_channels
...
async function handleRemoveGravbits(guildId, channelId) {
  try {
    await pgClient.query(`
      ...
      WHERE guild_id = $1 AND channel_id = $2
    `, [guildId, channelId]);
    console.log(`Removed channel ${channelId} from monitoring in guild ${guildId}.`);
  } catch (error) {
    console.error('Error removing channel:', error);
  }
}

// Setup interval for message deletion based on stored settings
async function setupIntervals() {
  try {
    const result = await pgClient.query("SELECT * FROM gravbits_channels");
    result.rows.forEach((row) => {
      const channel = bot.channels.cache.get(row.channel_id);
      if (channel) {
        setInterval(() => {
          handleAutoDelete(channel, row.delete_age);
        }, row.interval * 60000);
      }
    });
  } catch (error) {
...
// Function to automatically delete messages based on delete age
async function handleAutoDelete(channel, deleteAge) {
  try {
    const fetchedMessages = await channel.messages.fetch({ limit: 100 });
    const now = Date.now();
    const deleteBefore = now - (deleteAge * 60 * 1000);

    const messagesToDelete = fetchedMessages.filter(msg => msg.createdTimestamp < deleteBefore);
    
    if (messagesToDelete.size > 0) {
      await channel.bulkDelete(messagesToDelete, true);
      console.log(`Automatically deleted ${messagesToDelete.size} messages older than ${deleteAge} minutes in ${channel.name}`);
    }
  } catch (error) {
    console.error(`Error during automatic deletion in channel ${channel.id}:`, error);
  }
}
